...
export default function Badges({ data }: { data: Ebook }) {
  ...
  const BadgeChip = ({ type, value, text, icon, prefix, suffix, index }: Badge & { index: number }) => {
    ...
    return (
        ...
        >
          ...
          {type === 'number' && value && (
            <BadgeButton value={value} icon={Icon} prefix={prefix ?? undefined} suffix={suffix ?? undefined} formatted />
          )}
          {type === 'string' && text && (
            <BadgeButton value={text} icon={Icon} prefix={prefix ?? undefined} suffix={suffix ?? undefined} />
          )}
        </motion.div>
    )
  }
