...
const tabVariants = cva('relative overflow-hidden', {
	variants: {
		variant: {
			default: 'text-center shadow-sm flex divide-x',
			outline: 'outline outline-2 flex divide-x border-1',
			ghost:
				'shadow-none outline-none flex bg-transparent after:absolute after:bottom-0 after:w-full after:border-b-1 after:h-[2px] after:border-b-slate-500/40',
		},
		theme: {
			default:
...
const tabHighlightVariants = cva('absolute z-0', {
	...
	compoundVariants: [
		...
		},
		{
			...
			theme: 'default',
			className:
				'bg-transparent text-primary border-b-2 border-b-primary shadow-none',
		},
		{
			variant: 'ghost',
			theme: 'secondary',
			className:
				'bg-transparent text-secondary border-b-2 border-b-secondary shadow-none',
		},
		{
			variant: 'ghost',
			theme: 'tertiary',
			className:
				'bg-transparent text-tertiary border-b-2 border-b-tertiary shadow-none',
		},
	],
});
	...
	'inline-flex cursor-pointer items-center size-full justify-center whitespace-nowrap p-4 font-bold ring-offset-background transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 z-[1]',
	{
		variants: {
			...
			theme: {
				...
				secondary:
					'text-secondary-foreground data-[state=active]:text-secondary',
		...
		compoundVariants: [
			...
			},
			{
				...
				theme: 'secondary',
				className:
					'text-secondary data-[state=active]:text-secondary-foreground',
			},
			{
				variant: 'outline',
				theme: 'tertiary',
	...
	return (
		...
		>
			<TabsPrimitive.List
				...
				className={cn(
					'mb-10',
					tabVariants({ variant, rounded, theme, className }),
				)}
				{...props}
			>
				{children}
