...
export default defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      to: [{ type: "page" }],
      hidden: ({ parent }) =>
        !parent?.show_button || (parent?.show_button && parent?.button_link_type !== "INTERNAL") || parent?.disable_button,
      validation: (Rule) =>
        Rule.custom((field, context) =>
    ...
    defineField({
      ...
      type: "url",
      hidden: ({ parent }) =>
        !parent?.show_button || (parent?.show_button && parent?.button_link_type !== "EXTERNAL") || parent?.disable_button,
      validation: (Rule) =>
        Rule.uri({
          scheme: ["http", "https"],
